...
public:
    SchedulerPrivate() : mFreeBusyCache(nullptr)
    {
    }
...
bool Scheduler::acceptTransaction(const IncidenceBase::Ptr &incidence, iTIPMethod method, ScheduleMessage::Status status, const QString &email)
{
...
bool Scheduler::acceptPublish(const IncidenceBase::Ptr &newIncBase, ScheduleMessage::Status status, iTIPMethod method)
{
...
bool Scheduler::acceptRequest(const IncidenceBase::Ptr &incidence, ScheduleMessage::Status status, const QString &email)
{
    ...
    for (; incit != existingIncidences.end(); ++incit) {
        ...
        if (existingIncidence->revision() <= inc->revision()) {
            ...
            qCDebug(KCALUTILS_LOG) << "looking in " << existingIncidence->uid() << "'s attendees";
            ...
            if (isUpdate) {
                if (existingIncidence->revision() == inc->revision()
                    ...
                    qCDebug(KCALUTILS_LOG) << "This isn't an update - the found incidence was modified more recently";
                    deleteTransaction(existingIncidence);
                    return false;
                }
                qCDebug(KCALUTILS_LOG) << "replacing existing incidence " << existingIncidence->uid();
                bool res = true;
            ...
            qCDebug(KCALUTILS_LOG) << "This isn't an update - the found incidence has a bigger revision number";
            deleteTransaction(incidence);
            return false;
        }
    }
...
bool Scheduler::acceptCancel(const IncidenceBase::Ptr &incidence, ScheduleMessage::Status status, const QString &attendee)
{
    Incidence::Ptr inc = incidence.staticCast<Incidence>();
    if (!inc) {
        return false;
    ...
    for (; incit != existingIncidences.end(); ++incit) {
        ...
        qCDebug(KCALUTILS_LOG) << "looking in " << i->uid() << "'s attendees";
        ...
        if (isMine) {
            qCDebug(KCALUTILS_LOG) << "removing existing incidence " << i->uid();
...
bool Scheduler::acceptDeclineCounter(const IncidenceBase::Ptr &incidence, ScheduleMessage::Status status)
{
    Q_UNUSED(status);
    deleteTransaction(incidence);
    return false;
...
bool Scheduler::acceptReply(const IncidenceBase::Ptr &incidence, ScheduleMessage::Status status, iTIPMethod method)
{
    Q_UNUSED(status);
    ...
    if (ev || to) {
        ...
        qCDebug(KCALUTILS_LOG) << "match found!";
        ...
        for (inIt = attendeesIn.constBegin(); inIt != attendeesIn.constEnd(); ++inIt) {
            ...
            for (evIt = attendeesEv.constBegin(); evIt != attendeesEv.constEnd(); ++evIt) {
                ...
                if (attIn->email().toLower() == attEv->email().toLower()) {
                    ...
                    qCDebug(KCALUTILS_LOG) << "update attendee";
...
bool Scheduler::acceptFreeBusy(const IncidenceBase::Ptr &incidence, iTIPMethod method)
{
    ...
    qCDebug(KCALUTILS_LOG) << "freeBusyDirName:" << freeBusyDir();

    Person::Ptr from;
    if (method == iTIPPublish) {
        from = freebusy->organizer();
    } else if ((method == iTIPReply) && (freebusy->attendeeCount() == 1)) {
